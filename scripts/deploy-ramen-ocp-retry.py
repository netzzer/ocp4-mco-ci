...
def job(t):
    try:
        ...
        suffix = get_suffix()
        os.system(
            f"deploy-ocp multicluster 2  --cluster1 --cluster-name drcluster1-rdr-{suffix} --cluster-path /tmp/drcluster1-rdr-{suffix} --ocp4mcoci-conf samples/2_cluster_acm_setup/override_config.yaml --cluster2 --cluster-name drcluster2-rdr-{suffix} --cluster-path /tmp/drcluster2-rdr-{suffix}  --ocp4mcoci-conf samples/2_cluster_acm_setup/override_hub_config.yaml")
    except Exception:
        pass
