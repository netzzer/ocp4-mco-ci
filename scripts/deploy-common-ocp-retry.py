...
def jobnew(t):
    try:
        print("executing")
        suffix = get_suffix()
        os.system(f"deploy-ocp --ocp4mcoci-conf samples/deploy_ocp_cluster/override_config.yaml --cluster-name odfcluster-common-{suffix} --cluster-path /tmp/odfcluster-common-{suffix}")
    except Exception:
        pass
